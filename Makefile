...
publish: build/bin/$(narname)
	...
	cd $$(mktemp -d); git clone https://github.com/stas-badzi/doas-keepenv.git; cd doas-keepenv; git switch nix; cp $(current_dir)/build/bin/$(nixname) default.nix; git add default.nix; git commit -m "$(version)-$(release)"; git push
clean:
	rm -rf build/bin
